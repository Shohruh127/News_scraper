...
def test_ru_drafts_in_russian_then_says_it_in_uzbek(article, settings):
    ...
    assert RU_DRAFT["lead_uz"] in said_prompt and "fifth grade" in said_prompt
...
def test_the_russian_prompt_mirrors_the_uzbek_one_and_its_examples_render():
    ...
    assert "at most 7 sentences" in said and "Who reads it" in said
    assert len(said) < 2000, (
        "principles, not a rulebook: the owner measured the long version as a hindrance"
    )
